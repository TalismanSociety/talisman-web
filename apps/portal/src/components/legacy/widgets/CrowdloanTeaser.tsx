  ...
  ({ id, contributed, className }: { id: string; contributed?: boolean; className?: string }) => {
    ...
    return (
      <Link to={`/crowdloans/${parachainDetails?.slug ?? ''}`} className={`crowdloan-teaser ${className ?? ''}`}>
        <ParachainAsset id={parachainId ?? ''} type="card" />
        <div className="content">
          <div className="header">
            <ParachainAsset id={parachainId ?? ''} type="logo" />
            <CrowdloanBonus short id={id} prefix={<ParachainAsset id={parachainId ?? ''} type="logo" />} />
          </div>
          <h1>{parachainDetails?.name}</h1>
          <CrowdloanRaised id={id} title={t('Raised')} contributed={contributed} />
        </div>

        <Pill className="countdown">
          <CrowdloanCountdown id={id} showSeconds={false} />
        </Pill>
      </Link>
    )
  }
