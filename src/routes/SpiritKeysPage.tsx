...
import { Button, Panel, PanelSection, Poster, useModal, Field } from '@components'
...
import {fetchNFTData} from '../archetypes/SpiritKeys/NFT'
  ...
  return (
    <section className={className}>
      ...
      <div className="content">
         <img className="spirit-key-image" src={(totalNFTs.length > 0) ? (totalNFTs[currentNFT]?.image?.replace('ipfs://', 'https://rmrk.mypinata.cloud/')) : baseImage}/>
         <p>You have {(totalNFTs.length > 0) ? totalNFTs.length : 'no'} Spirit Keys</p>
         {totalNFTs.length < 1 && (
            <div className="empty-state-buttons-div">
               <a href='https://discord.gg/jTzkMY9Y' target="_blank"><Button className="join-discord-button"> Join Discord</Button></a>
               <Button to="/crowdloans" className="explore-crowdloans-button"> Explore Crowdloans </Button>
            </div>
         )}
         {totalNFTs.length > 0 && (
         <div className="spirit-key-body">
            ...
            <div className="address-input-container">
               ...
               />
               <button
                  onClick={(e:any) => {SendNFT(totalNFTs[currentNFT], recipient)}}
               >
                  Send
               </button>
            </div>
         ...
         <article>
            <h1>Talisman Seeker, it's your spirit that's key.
