...
export const useStatus = (props={}) => {
  const {
    status=baseStatusOptions.INITIALIZED, 
    message=null,
    customOptions={}
  } = props
  ...
  // set initial
  const [_status, setStatus] = useState((!!status && Object.keys(statusOptions).includes(status)) ? statusOptions[status] : statusOptions.INITIALIZED)
