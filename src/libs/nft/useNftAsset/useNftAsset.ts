...
function getNftCollectibleUrl(nft: any) {
  const url =
    RMRK1.collectibleUrl(nft?.id, nft?.srcUrl) ||
    RMRK2.collectibleUrl(nft?.id, nft?.srcUrl);
  return url;
}
...
export function useNftAsset(nft: any) {
  ...
  const contentExtension =
    prefetchedContentExtension || contentExtensionFromMetadata;
  const previewAnimationUrl =
