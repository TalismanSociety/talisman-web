...
export function useNfts(addresses: string[], options?: UseNftsOptions) {
  ...
  const limitNfts = options?.limit
    ? (nftsWithSrc || []).flat().slice(0, options?.limit)
    : null;
  const limitNftsPerAddress = options?.limitPerAddress
    ? (nftsWithSrc || []).flatMap((flatData) =>
        (flatData || []).slice(0, options?.limitPerAddress)
      )
    : null;
  return {
