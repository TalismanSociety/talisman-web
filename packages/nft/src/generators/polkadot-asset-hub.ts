...
import { graphql } from '../../generated/gql/polkadot-asset-hub/index.js'
import type { CreateNftAsyncGenerator, Nft } from '../types.js'

export const createPolkadotAssetHubNftAsyncGenerator: CreateNftAsyncGenerator<Nft<'asset-hub', 'polkadot'>> =
  async function* (address, { batchSize }) {
    ...
    while (true) {
      ...
      yield* response.collections.flatMap(collection =>
        collection.nfts.map((nft): Nft<'asset-hub', 'polkadot'> => {
