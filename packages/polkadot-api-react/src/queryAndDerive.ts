...
import { RecoilState, RecoilValueReadOnly, atomFamily, constSelector, errorSelector, type RecoilValue } from 'recoil'
import { Observable } from 'rxjs'
import { ChainStorageStateContext, EndpointContext } from './contexts.jsx'

type Config = {
  getApiPromise: (endpoint: string) => ApiPromise | PromiseLike<ApiPromise> | RecoilValue<ApiPromise>
}

export const chainStorageAtomFamily = ({ getApiPromise }: Config) =>
  atomFamily({
    key: '@talismn/polkadot-api-react/ChainStorage',
    effects: ([endpoint, typeName, moduleName, sectionName, params]: [string, string, string, string, any[]]) => [
      ({ setSelf, getPromise }) => {
        const boxedApiPromise = getApiPromise(endpoint)

        const apiPromise =
          boxedApiPromise instanceof ApiPromise
            ? Promise.resolve(boxedApiPromise)
            : 'then' in boxedApiPromise
            ? boxedApiPromise
            : getPromise(boxedApiPromise)
...
>(
  ...
  const chainStorageState = useContext(ChainStorageStateContext)
  ...
  return chainStorageState([endpoint, 'query', String(moduleName), sectionName, params]) as TReturn
}
...
>(
  ...
  return chainStorageState([endpoint, 'derive', String(moduleName), sectionName, params]) as any as TReturn
}
