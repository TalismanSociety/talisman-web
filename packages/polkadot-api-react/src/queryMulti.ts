...
import { RecoilValueReadOnly, atomFamily, constSelector, errorSelector, type RecoilValue } from 'recoil'
import { Observable } from 'rxjs'
import { ChainQueryMultiStateContext, EndpointContext } from './contexts.jsx'
...
export const chainQueryMultiAtomFamily = ({ getApiPromise }: Config) =>
  atomFamily({
    key: '@talismn/polkadot-api-react/QueryMulti',
...
>(
  ...
  const chainQueryMultiState = useContext(ChainQueryMultiStateContext)
  ...
  return chainQueryMultiState({ endpoint, queries }) as any as TReturn
}
